...
. _SECRET_PEM_TARGET_
.. **Definition** -> The location of the personal TLS key-certificate combination of the server in the test network.
.. **Schema** -> Must be a UNIX absolute filepath.
...
. _DATASTORE_FILEPATH_
.. **Definition** -> The filesystem location where the datastore should save logs.
.. **Schema** -> Must be a UNIX absolute filepath.

. _DATASTORE_DEFAULT_FILEPATH_
.. **Definition** -> The filesystem location where the datastore should save logs.
.. **Schema** -> Must be a UNIX absolute filepath.
...
. _DATASTORE_PASSWORD_
.. **Definition** -> The password of the account for accessing the datastore.
.. **Schema** -> Must be a complex, decently long string with no spaces. Optional.

. _DATASTORE_OPERATIONS_FILEPATH_
