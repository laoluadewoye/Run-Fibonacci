...
`test_image.ps1` is a PowerShell script used to test the image on Windows systems.

`test_image.sh` is a Shell script used to test the image on Linux systems.

`TestGenTLS.py` is a Python script used to create the TLS materials for the test_image scripts.
