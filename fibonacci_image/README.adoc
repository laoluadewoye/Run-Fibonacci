...
. Curl
. Python (version 3.13 is recommended)
...
. The image defines the `docker.io/library/python:3.13-alpine3.22` image as a base layer.
...
`Dockerfile` is the file that defines the image.
...
`testing/test_image.ps1` is a PowerShell script used to test the image on Windows systems.

`testing/test_image.sh` is a Shell script used to test the image on Linux systems.

`testing/TestGenTLS.py` is a Python script used to create the TLS materials for the test_image scripts.

`testing/remove_test.ps1` is a PowerShell script used to remove test items on Windows systems.

`testing/remove_test.sh` is a Shell script used to remove test items on Linux systems.
