...
.. **Schema** -> Must be one of a set of constants defined for the `server.api` key in the project README.
.. **Default** -> "rest"

. _datastore.auth.username_
.. **Definition** -> The username of the account for accessing the datastore.
.. **Schema** -> Must be a string with alphanumerical characters with no spaces. Optional.
.. **Default** -> ""

. _datastore.auth.password_
.. **Definition** -> The password of the account for accessing the datastore.
.. **Schema** -> Must be a complex, decently long string. Optional.
.. **Default** -> ""

. _datastore.logs.defaultPath_
.. **Definition** -> The default filesystem location where the datastore should save server logs.
.. **Schema** -> Must be a UNIX absolute filepath.
.. **Default** -> "/tmp/default.csv"

. _datastore.logs.operationPath_
.. **Definition** -> The filesystem location where the datastore operations that can't be sent should be saved.
.. **Schema** -> Must be a UNIX filepath.
.. **Default** -> "/tmp/operations.csv"

. _datastore.logs.serverPath_
.. **Definition** -> The primary filesystem location where the datastore should save server logs.
.. **Schema** -> Must be a UNIX filepath.
.. **Default** -> "/tmp/datastore.csv"

. _datastore.type_
.. **Definition** -> The type of Datastore to use.
.. **Schema** -> Must be one of a set of constants defined for the `server.datastore` key in the project README.
.. **Default** -> "none"

. _network.datastore.address_
...
. _network.self.address.healthcheck_
.. **Definition** -> The network address the server uses to call itself for a healthcheck in the test network.
.. **Schema** -> Must be either a IPv4 address or a FQDN.
.. **Default** -> "127.0.0.1"

. _network.self.address.listening_
...
. _tls.ca.keyPath_
.. **Definition** -> The location of the certificate authority TLS key of the server in the test network.
.. **Schema** -> Must be a UNIX filepath.
.. **Default** -> "./ca.key"
...
. _tls.gen.caSuffix_
.. **Definition** -> The suffix to append to generated PEM files containing both the server's and CA's certificate.
.. **Schema** -> Must be a string with letters, dashes, and/or hyphens.
.. **Default** -> "ca"

. _tls.gen.certDays_
.. **Definition** -> The length that the server's created certificate should be active.
.. **Schema** -> Must be a number that can be turned into a Python integer.
.. **Default** -> 1

. _tls.gen.ext.cert_
.. **Definition** -> The file extension to use for TLS certificates.
.. **Schema** -> Must be a valid extension.
.. **Default** -> "crt"

. _tls.gen.ext.key_
.. **Definition** -> The file extension to use for TLS keys.
.. **Schema** -> Must be a valid extension.
.. **Default** -> "key"

. _tls.gen.ext.pem_
.. **Definition** -> The file extension to use for TLS certificate-key combinations.
.. **Schema** -> Must be a valid extension.
.. **Default** -> "pem"

. _tls.gen.keyLength_
.. **Definition** -> The bit length of the RSA key.
.. **Schema** -> Must be a number that can be turned into a Python integer.
.. **Default** -> 4096

. _tls.gen.pubExponent_
.. **Definition** -> The public exponent of the RSA key.
.. **Schema** -> Must be a number that can be turned into a Python integer.
.. **Default** -> 65537

. _tls.gen.secretTarget_
.. **Definition** -> The filepath (minus the file extension) that should be used to name created server TLS materials.
.. **Schema** -> Must be a UNIX filepath.
.. **Default** -> "./self"

. _tls.san.ips_
.. **Definition** -> The list of IP addresses to use as SANs.
.. **Schema** -> Must be an IP address or a string formatted as IP addresses separated with a comma.
.. **Default** -> "127.0.0.1"

. _tls.san.names_
.. **Definition** -> The list of FDQNs to use as SANs.
.. **Schema** -> Must be an FQDN or a string formatted as FQDNs separated with a comma.
.. **Default** -> "localhost"
...
. _workers_
.. **Definition** -> The number of server workers to create.
.. **Schema** -> Must be a number that can be turned into a Python integer.
.. **Default** -> 3
