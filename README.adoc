...
...... **Schema** -> JSON string
... _namespacePolicy_
.... **Definition** -> `kube` member key that specifies the security policy for namespaces.
.... **Schema** -> JSON string
... _podRestartPolicy_
.... **Definition** -> `kube` member key that specifies the restart policy for pods.
.... **Schema** -> JSON string
... _replicas_
.... **Definition** -> `kube` member key that specifies the number of replica pods to make for a given server stage.
.... **Schema** -> JSON integer
. _platform_
